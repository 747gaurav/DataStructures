...
void list::inslast(int x)
{
	...
	cout<<endl<<" Inserted successfully at the end..";
	disp();
}
...
void list:: insbeg(int x)
{
	...
	p->data=x;
	p->link=q;
	cout<<endl<<" Inserted successfully at the begining..";
	disp();
}
...
void list::delelement(int x)
{
	...
	cout<<endl<<" Element u entered   "<<x<<"    is not found..";
}
...
void list::swap()
{
	...
	if(s == NULL)
	{
		cout<<"Only 1 elemet in list. Second element is NULL"<<endl;
		return;
	}
	nhead = s;
	while(s != NULL)
...
do
{
	...
	{
	case 1:
		cout<<"1.Insertion at begining"<<endl<<"2.Insertion at the end"<<endl;
...
//		getch();
	...
	case 5:
		...
		cout<<endl<<"List before SWAP ="<<endl;
		l->disp();
		cout<<endl<<"List after SWAP ="<<endl;
		l->swap();
		break;
	case 6:
		for(int i = 1;i<=4;i++)
